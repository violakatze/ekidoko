...
- 出典: 「国土数値情報（鉄道データ）」（国土交通省）2024年 全国データ（railroad.pmtilesと同一ソース）
  - https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N02-2024.html

| プロパティキー | 内容 |
|---------------|------|
| `N02_005` | 駅名 |
| `N02_005g` | グループ駅名（正解判定に使用） |
| `N02_003` | 路線名 |
| `N02_004` | 会社名 |
...
- 出典: 「国土数値情報（行政区域データ）」（国土交通省）2025年 全国データ
...
- 内容: 日本全国の鉄道路線（ライン）
- OpenLayersで `ol-pmtiles` を使い VectorTileLayer として描画する
- 出典: 「国土数値情報（鉄道データ）」（国土交通省）2024年 全国データ
  - https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N02-2024.html
...
| モード | 内容 |
|--------|------|
| レベル1 | OSM背景あり、都道府県レイヤーなし |
| レベル2 | OSM背景なし、都道府県レイヤーあり |
| レベル3 | OSM背景なし、都道府県レイヤーなし |
| 閲覧モード | 出題なし、OSM背景あり、都道府県レイヤーなし、ホバー情報表示 |
...
- **正解判定**: クリックした駅の `N02_005g`（グループ駅名）が正解駅の `N02_005g` と一致した場合に正解とする
  - 同じ `N02_005g` を持つ駅が複数ある場合（乗換駅など）はいずれをクリックしても正解
- 正解: 同じ `N02_005g` の駅を全て緑でハイライト → 「次の問題へ」ボタンを表示 → 押下で次の問題へ進む
- 不正解（回答機会が残っている場合）: クリックした駅を赤くハイライト表示、回答機会を1減らす
- 不正解確定（3回目の誤答、または60秒タイムアウト）: 正解駅（同じ `N02_005g` の駅全て）をハイライト表示 → 「次の問題へ」ボタンを表示 → 押下で次の問題へ進む
...
**駅レイヤー**
- 塗り色: `#ffffff`（白）、線幅: 6px
- 縁取り: `#888888`（グレー）、線幅: 9px（白の下に重ねて縁取りとして表示）
- ストロークを路線より太く表示する（路線: 2px、駅: 6px 程度）

**正解ハイライト**
- 正解駅（同じ `N02_005g` を持つ駅全て）: 緑（`#00C853`）で強調表示
...
type StationFeature = {
  ...
  stationGroupName: string;  // N02_005g（正解判定に使用）
  lineName: string;          // N02_003
  companyName: string;       // N02_004
  lineCoordinates: [number, number][]; // LineStringの頂点座標列
  centerCoordinates: [number, number]; // 地図移動用の中心座標
  featureId: string;
};
...
type QuestionState = {
  ...
  resolved: boolean;         // 回答確定済みか
  correct: boolean;
  wrongFeatureIds: string[]; // 誤クリックした駅IDの一覧
};
